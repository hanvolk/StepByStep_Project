/*
     ...
     프로그램의 기능을 담당하는 컨트롤 클래스 AccountHandler 함수들의 정의
*/
#include "BankingCommonDel.h"
#include "AccountHandler.h"
#include "Account.h"

void AccountHandler::ShowMenu(void) // 전체 메뉴 출력
{
    system("cls");
    cout<<"===== Menu ====="<<endl;
    cout<<"1. 계좌개설"<<endl;
...
void AccountHandler::MakeAccout(void)  // 계좌 생성
{
    int id;
    char name[NAME_LEN];
    int balance;
...
void AccountHandler::DepositMoney(void) // 입금 기능
{
    int money;
    int id;
    cout<<"[입   금]"<<endl;
...
void AccountHandler::WithdrawMoney(void) // 출금 기능
{
    int money;
    int id;
    cout<<"[출   금]"<<endl;
...
void AccountHandler::ShowAllAccInfo(void) // 전체 계좌 정보 출력
{
...
void AccountHandler::Exit(void) // 프로그램 종료
{
    for(int i=0;i<accNum;i++)
            delete accArr[i];
}

void AccountHandler::StoreAccData(void) // 데이터의 저장
{
...
void AccountHandler::LoadAccData(void) // 데이터의 로드
{
    int i=0;
